...
import varys.framework.{FrameworkMessage, Failure, Success}
...
//    val akkaWriteTimeout = System.getProperty("varys.akka.writeTimeout", "30").toInt
  ...
  def tellActor(actor: ActorRef, message: Any) {
    askActorWithReply[FrameworkMessage](actor, message) match {
      case Success =>
...
//  def getActorRef(url: String, actorSystem: ActorSystem): ActorRef = {
