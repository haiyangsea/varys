...
import varys.framework.serializer.JavaNativeSerializer
import varys.framework.{DataType, FileFlowDescription, GetRequest, NioDataServer}
...
class NioThrottledInputStreamSuit extends FunSuite with BeforeAndAfter{
  ...
  val serializer = new JavaNativeSerializer
  before {
    dataServer.start()
  }
  ...
  test("nio throttled input stream") {
    ...
    val flowDesc = new FileFlowDescription("0", path, "0", DataType.ONDISK, 0, size, 1, "10.177.35.224", 0)
